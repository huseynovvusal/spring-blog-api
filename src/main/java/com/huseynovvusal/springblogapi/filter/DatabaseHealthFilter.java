...
/**
 * Filter that checks database connection status and provides appropriate response during service
 * failure Must be executed after BlockedUserFilter
 */
@Slf4j
@Component
...
public class DatabaseHealthFilter extends OncePerRequestFilter {
  ...
  private void sendServiceUnavailableResponse(HttpServletResponse response, String path)
      throws IOException {
    ...
    errorResponse.put(
        "message",
        "Service is temporarily unavailable due to database connection failure. Please try again later.");
